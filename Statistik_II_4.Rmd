...
* Oft nutzen Psychologen die einfache lineare Regression, um eine intervallskalierte AV $(Y)$ mit einer intervallskalierten UV $(X)$ vorherzusagen
...
$$ b=\frac{\sigma_{xy}^2}{\sigma^2_x}=\frac{`r round(cov(df2$Gruppe, df2$Sorgen), 2)`}{`r round(var(df2$Gruppe, na.rm = T),2)`} = `r round(cov(df2$Gruppe, df2$Sorgen) / var(df2$Gruppe, na.rm = T), 2)`$$
...
$\rightarrow$ t-Wert und p-Wert sind identisch!
...
* Ihre Anwendung ist also prinzipiell nur Modellierung solcher Zusammenhänge angemessen.

Beispiele für bivariate (zwischen 2 Variablen) non-lineare Zusammenhänge:

  * Eponentieller Zusammenhang
...
* Prüfgröße ist t-verteilt mit $df=N-2$ Freiiheitsgraden
...
Für die Berechnung des Standardfehlers der Steigung $(b_{xy})$ ermitteln wir den Standardschätzfehler:
...
$$ b=\frac{\sigma_{xy}^2}{\sigma^2_x}=\frac{`r round(cov(df$IQ, df$Testleistung), 2)`}{`r round(var(df$IQ, na.rm = T),2)`} = `r round(cov(df$IQ, df$Testleistung) / var(df$IQ, na.rm = T), 2)`$$
...
$\beta_{xy}=0.12,$ $KI_{95\%}[0.05 - 0.19]$

$\rightarrow$ Da das KI den Wert 0 nicht umschließt, ist $\beta_{xy}$ signifikant.
...
1. Das Kriterium (AV) muss intervallskaliert und nnormalverteilt sein.
