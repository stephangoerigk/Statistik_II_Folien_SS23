...
* Jetzt wollen wir einen Schritt weiter gehen und unterstellen dass der Zusammenhang eine Richtung hat
...
* Die Intensität des Sports wirkt sich den Puls aus aber nicht umgekehrt

* Wenn wir so einen gerichteten Zusammenhang untersuchen dann stellt sich die Frage wo die Richtung herkommt

$\rightarrow$ Typischerweise müssen wir die aus der Theorie oder aus Plausibilitätsüberlegungen herleiten 
...
* Wir wollen aber genau die Geraden finden, welche die Daten am allerbesten beschreibt.
]
...
$$b=\frac{s^2_{XY}}{s^2_X} = r_{XY} \cdot \frac{s_y}{s_x}$$
]
...
<br>
<br>
Kovarianz von XY = $s_{xy}=\frac{\displaystyle \sum_{i=1}^{n}(x_{i}-\bar{x})\cdot(y_{i}-\bar{y})}{n-1}^2$ 
<br>
<br>
Varianz von X =  $s^2=\dfrac{\displaystyle\sum_{i=1}^{n}(x_{i}-\bar{x})^2}{n-1}$ 
...
<br>
<br>
Kovarianz von XY = $s_{xy}=\frac{\displaystyle \sum_{i=1}^{n}(x_{i}-\bar{x})\cdot(y_{i}-\bar{y})}{n-1}^2$ = `r round(cov(sampledata$X, sampledata$Y), 2)`
<br>
<br>
Varianz von X =  $s^2=\dfrac{\displaystyle\sum_{i=1}^{n}(x_{i}-\bar{x})^2}{n-1}$ =  `r round(var(sampledata$X, na.rm = T),2)`
...
$$ b=\frac{s_{xy}}{s^2_x}=\frac{`r round(cov(sampledata$X, sampledata$Y), 2)`}{`r round(var(sampledata$X, na.rm = T),2)`} = `r round(cov(sampledata$X, sampledata$Y) / var(sampledata$X, na.rm = T), 2)`$$
...
$$R^2=\frac{\text{erklärte Varianz}}{\text{gesamte Streuung}}=\frac{\sum\limits _{i=1}^{n}(y_i-\bar{y}_i)^2}{\sum\limits _{i=1}^{n}(\hat{y}_i-\bar{y}_i)^2}=(\frac{s_{XY}}{s_X \cdot s_Y})^2$$
<br>
...
* Ein **signifikantes Ergebnis** der ANOVA bedeutet lediglich, dass sich zumindest eine Stude des Faktors von einer anderen unterscheidet.
