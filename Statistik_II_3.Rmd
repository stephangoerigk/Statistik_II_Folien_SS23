...
<br>
<br>
Kovarianz von XY = $\sigma_{xy}^2=\frac{\displaystyle \sum_{i=1}^{n}(x_{i}-\bar{x})\cdot(y_{i}-\bar{y})}{n-1}^2$ 
<br>
<br>
...
<br>
<br>
Kovarianz von XY = $\sigma_{xy}^2=\frac{\displaystyle \sum_{i=1}^{n}(x_{i}-\bar{x})\cdot(y_{i}-\bar{y})}{n-1}^2$ = `r round(cov(sampledata$X, sampledata$Y), 2)`
<br>
<br>
...
- a ist also der Wert wo $X=0$ ist, also hat jemand mit 0 auf der UV einen AV Wert von 29 hat
...
* Regressionsgleichung ist ddefiniert durch die **Regressionskoeffizienten** (Y-Achsenabschnitt und Steigung), welche aus Daten geschätzt werden müssen.
