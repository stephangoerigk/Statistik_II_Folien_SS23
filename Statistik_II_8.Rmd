...
##### 14.06.2023 | Prof. Dr. phil. Stephan Goerigk
...
Wir müsse uns wiederum fragen, weshalb Messungen unterschiedlich (mit Varianz) ausfallen
...
$$\sigma_{gesamt}^2= \sigma_{Vpn}^2 + \sigma_{Effekt}^2 +  \hat{\sigma}_{Res}^2$$  
...
df[["P(m)"]] = round(rowMeans(df[,2:4]), 3)
...
* Die Varianzschätzungen der ANOVA mit Messwiederholung gehen von eine Interaktion der Messwiederholung mit unspezifischen Personencharakteristika aus
...
* Erfolgt über die Abweichung der gemessenen Werte von den, allein auf Grund von
  1. der Mittelwerte zu jedem Zeitpunkt
  2. der Personenmittelwerte
  zu **erwartenden** Werten $(x_{im(erwartet)})$
...
* Sie wird somit aus de Varianz der Wechselwirkung zwischen Messwiederholungsfaktor und Personenfaktor geschätzt
...
$$\hat{\sigma}^2_{A\text{ x }Vpn}  = \frac{[9 - (10.4 + 12.75 - 15.6)]^2 + ..[19 - (19.8 + 12.25 - 15.6)]^2}{(3-1) \cdot (5-1)} = \frac{23.6}{8}=2.95$$
...
* Püfung of sich die Messzeitpunkte signifikant unterscheiden

* F-Bruch (emp. F-Wert) wird gebildet aus geschätzter systematische Varianz für Messwiederholungsfaktor (A) und der geschätzten Residualvarianz 
...
* Der Vergleich von Effektstärke pber Studien hinweg kann problematisch sein, wenn Korrelationen zwischen Messungen variieren.
...
* Wenn Spärizität erfüllt ist, können Post-Hic Vergleiche mittels **Tukey-Test** geprüft werden.
]
]
