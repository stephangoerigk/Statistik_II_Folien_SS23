...
Termine:
* 14 Termine
* Mittwoch 12:20 - 13:50 (Audimax)

Begleitendes Seminar:
* Gruppe A: Mittwoch 13:50-14:35 (SR08, Pavillon 2)
* Gruppe B: Mittwoch 14:45-15:30 (SR08, Pavillon 2)
* Dozentin: Sara Vragolic ([sara.vragolic@charlotte-fresenius-uni.de](mailto:sara.vragolic@charlotte-fresenius-uni.de))

Materialien:
* werden auf [Studynet](https://studynet.hs-fresenius.de/ilias.php?ref_id=15322&cmd=view&cmdClass=ilobjgroupgui&cmdNode=xa:o1&baseClass=ilRepositoryGUI) bereitgestellt
...
* Erweiterung durch eine weitere Behandlungsoption (z.B. Medikament 2)

* Forschungsfragen: 

  * Unterscheiden sich die 3 Mittelwerte voneinander?
  * Welche Gruppe ist welcher überlegen (hat den höchsten/niedrigsten Mittelwert)?
...
* Wir kennen bereits den t-Test
  ...
  * t-Wert prüft, wie wahrscheinliche eine Mittelwertsdifferenz unter Annahme der $H_0$ ist 

.pull-left[
**FRAGE:**

* Können wir mit dem t-Test auch mehr als 2 Mitttelwerte vergleichen?
...
$\rightarrow$ Ist zwar mit steigender Anzahl Vergleiche immer aufwendiger, aber man könnte beim bekannten Verfahren bleiben!
]
.pull-right[
...
* Es werden alle Mittelwerte auf einmal miteinander verglichen.
...
* Die $\alpha$ -Fehlerkumulierung tritt nur dann auf, wenn mehrere Tests für eine Hypothese an den selben Daten durchgeführt werden
...
* mehrere t-Tests $\rightarrow$ es gehen immer nur Teile der Gesamtstichprobe in die Analyse ein
...
* Name der Varianzanalyse:
  ...
  * Simultanvergleich der Mittelwerte wird über Betrachtung verschiedener Varianzen erreicht
  
  * Aus dem Vergleich der Varianzen wird die Entscheidungsregel abgeleitet

Historisch:

* Geht zurück auf Sir Ronald Fisher

* Versteht ANOVA als Vergleich systematischer Einflüsse (systematische Varianz) und unsystematischer Einflüsse (Residualvarianz)

* Quotient der beiden Varianzen ergibt den F-Wert (Teststatistik - ähnlich t-Wert)
...
df = data.frame(Placebo = c(6,7,7,8),
                ...
                `Medikament 2` = c(NA, 12, 12, 13))
kable(df[,])
```
]

.pull-right[
Verbesserung scheint sich zwischen Gruppen zu unterscheiden:
...
Größe der Unterschiede zwischen den gemessenen Werten = Varianz
...
$$QS_x=\sum\limits _{i=1}^{n}(x_{i}-\bar{x})^2; df_x=n-1$$
...
* Gibt an, wie stark sich alle Personen voneinander unterscheiden

* Berechnung: Jeder Wert in Varianzformel vom Gesamtmittelwert $(\bar{G})$ abziehen
...
* Können auch natürliche Gruppenzugehörigkeiten sein (z.B. Geschlecht, etc.)
]
...
* Unterschiede zwischen Personen, unabhängig von Manipuation

* können zeitlich stabil oder instabil sein (z.B. Motivation, Tagesstimmung, Persönlichkeit, Messfehler...)

$\rightarrow$ nennt man auch Residualvarianz
]
...
$\rightarrow$ Quadrierte Abweichung jedes Messwerts von seinem Gruppenmittelwert

<small>
Good-to-know: "Varianz innerhalb" Zieht sich streng genommen auf die Varianzen innerhalb jeder einzelnen Gruppe. Der Begriff wird jedoch auch synonym für die Residualvarianz, also die über alle Gruppen hinweg gemittelte Varianz innerhalb verwendet
</small>
...
Folge: Die Berechnung der **durchschnittlichen Varianz innerhalb** erfolgt durch die Addition der Varianzen innerhalb der einzelnen Gruppen geteilt durch die Anzahl der Gruppen
...
Diese Darstellung ermöglicht die separate Betrachtung von Quadratsummen und Freiheitsgraden:
...
Für die anderen beiden Gruppen ergibt sich (ann nachgerechnet werden):
...
* Jeder eEsswert wird mit dem eigenen Gruppenmittelwert verglichen

$\rightarrow$ Die Gruppen werden unabhängig betrachtet und der Durschnitt aus ihren Varianzen gebildet (Varianz innerhalb soll nur unsystematische Einflüsse erfassen)
...
* Experimenteller Einfluss auf die Gesamtvarianz wird pber die Unterschiede der Gruppenmittelwerte geschätzt
...
$$F_{(df_{Zähler}; df_{Nenner})}=\frac{\hat{\sigma}_{zwischen}}{\hat{\sigma}_{innerhalb}}$$
...
Mögliche Werte und Interpretation:
...
* Unter der $H_0$ hat die F-Verteilung einen Mittelwert von 1 (F=1)
  ...
  $\rightarrow$ bei Gleichheit der Populationsmittelwerte besteht **Varianz zwischen** nur auch Residualvarianz und nimmt gleichen Wert wie **Varianz innerhalb** an
  
* Negative Werte sind nicht möglich, da Varianzen durch Quadrierung imer positiv sind
]
...
* Wahrscheinlichkeit für einen F-Wert (in Abhängigkeit von Gruppenzahl und Stichprobengröße) kann in der F-Tabelle nachgelesen werden

* Logik: Vergleich empirischer F-Wert $(F_{emp})$ mit kritischem F-Wert $(F_{krit})$ unter Annahme des Signifikanzniveaus $(\alpha=.05)$

Wenn $F_{emp} > F_{krit}$ $\rightarrow$ ist ANOVA signifikant.
]
...
* Aus Messwerten werden inner- und zwische Varianz sowieder F-Quotient
...
* Zwischenvarianz größer als Varianz innerhalb

* Manipulation verursacht signifikanten Teil der Gesamtvarianz

* Mit anderen Worten: Es besteht ein signifikanter (systematischer) Unterschied zwischen den Gruppen

* ACHTUNG: Nur statistische Signifikanz $\rightarrow$ für Relevanz auf der inhaltlichen Ebene, müssen Effektstärken berücksichtigt werden

**VORSICHT:**

* Eine signifikante ANOVA bedeutet lediglich, dass sich mindestend ein Gruppenmittelwert signifikant von den anderen unterscheidet 
...
Unsere Stichprobe ist auf insgesamt $N=150$ (n=50 pro Gruppe) erweitert worden:
...
```{r}
psych::describe.by(df[,c("Symptome", "Gruppe")], "Gruppe")
```
...
* $\hat{\sigma}_{zwischen}^2=\frac{QS_zwischen}{df_{zwischen}}=\frac{582.4}{2}=291.2$
* $\hat{\sigma}_{innerhalb}^2=\frac{QS_innerhalb}{df_{innerhalb}}=\frac{2082.2}{147}=14.17$
