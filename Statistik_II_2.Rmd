...
* Signifikante ANOVA: Es ist lediglich gesichter, dass Gruppe mit kleinstem Mittelwert sich signifikant von Gruppe mit größtem Mittelwert unterscheidet.
...
* Wie groß muss die Differenz zwischen zwei Gruppenmittelwerten sein, damit diese auf einem kumulierten $\alpha$-Niveau signifikant ist, dass nicht die zuvor festgelegte Grenze überschreitet (i.d.R. 0.05)?
...
table_df %>%
  kbl() %>%
  kable_classic(full_width = T, position = "left", font_size = 8)
```
...
$\rightarrow$ In der Tabelle der $q_{krit}$-Werte verwenden wir die nächst kleinereenthaltene Anzahl an Fehlerfreiheitsgraden $df_{innerhalb}=120$
