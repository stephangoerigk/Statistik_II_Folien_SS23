...
* Deshalb interesiert oft die Wirkung von nicht einem, sondern mehreren Faktoren auf die AV
...
* Sie quantifiziert Einflüsse, die nur durch die gemeinsame, simultane Wirkung zweiter Faktorstufen entstehen $\rightarrow$ nicht durch die Haupteffekte alleine
...
* Formell: Interaktion betrachtet Abweichungen, der beobachteten Zellmittelwerte von den aufgrund der Haupteffekte zu erwartenden Tellmittelwerte
...
$$\hat{\sigma}^2_{UV_1}= \frac{\sum\limits _{i=1}^{p}\sum\limits _{j=1}^{1}n \cdot [\bar{UV_{1}UV_{2}}_{ij} - (\bar{UV_{1i}} + \bar{UV_{2j}}+ \bar{G})]^2}{(p-1 \cdot (q-1)}$$
---
class: top, left
### Mehrfaktorielle ANOVA
...
$$\hat{\sigma}^2_{UV_1}= \frac{\sum\limits _{i=1}^{p}\sum\limits _{j=1}^{1}n \cdot [\bar{UV_{1}UV_{2}}_{ij} - (\bar{UV_{1i}} + \bar{UV_{2j}}+ \bar{G})]^2}{(p-1 \cdot (q-1)}$$
$$\bar{G}= `r round(mean(df$Verbesserung), 2)`$$
$$\hat{\sigma}^2_{UV_1}= \frac{4 \cdot [11.75  - (16 + 18 - 22.25)]^2 + ... + [32.75  - (28.5 + 26.5 - 22.25)]^2}{(2-1 \cdot (2-1)}=196$$
